...
export async function ensureScheduledDomainFresh({ key, env, prepare, latestRun, sync, maxAgeMinutes }) {
  await prepare();
  const run = await latestRun();
  if (!needsFreshnessRecovery(run, maxAgeMinutes)) return { triggered: false, run };
  return inFreshnessFlight(key, async () => {
    const latest = await latestRun();
    if (!needsFreshnessRecovery(latest, maxAgeMinutes)) return { triggered: false, run: latest };
