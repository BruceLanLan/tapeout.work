...
export async function v1(request, env) {
  ...
  if (url.pathname === "/api/v1/openapi.json") return json({
    ...
    paths: {
      ...
      "/api/v1/ecosystem/health": { get: { summary: "Self-probed HEAD/GET reachability check of every curated ecosystem tool's own URL, at most once per 60 minutes per tool. Not uptime history, content correctness or an endorsement of the linked tool." } }, "/api/v1/changelog": { get: { summary: "When this catalogue changed, derived at build time from the public git history of the seed and locale files. A record of when a change landed and what its commit said it changed, not a diff of the entries and not a claim that every change was editorially significant." } }, "/api/v1/self-audit": { get: { summary: "The monitor auditing its own catalogue: which catalogued tool pages have shipped a change since we last reviewed their description, plus invariant checks over localisation coverage, evidence fields and review age. Detects that a page changed, never what changed or whether our description is now wrong; nothing is written into the catalogue automatically." } },
  ...
  // Built at deploy time from git history — a Worker has no repository to read —
  // and served from assets so the catalogue evolution is checkable, not just asserted.
  // A missing file means the build step did not run, which is said plainly rather than
  // answered with an empty list that would read as "nothing ever changed".
  if (url.pathname === "/api/v1/changelog") {
    const asset = await env.ASSETS.fetch(new Request("https://tapeout.work/changelog.json"));
    if (!asset.ok) return json({ status: "unavailable", reason: "changelog.json was not generated for this deployment; run scripts/build_changelog.mjs before deploying" }, 503, { "cache-control": "no-store" });
    return json(await asset.json(), 200, { "cache-control": "no-store" });
  }
