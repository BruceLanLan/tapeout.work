export const ECOSYSTEM_CATALOG_VERSION = "2026-09-06b";
...
export const CURATED_TOOLS = Object.freeze([
  ...
  },
  {
    ...
    original_language: "zh",
    reviewed_at: "2026-09-05T18:25:00Z",
    title_en: "TapeOutScan explorer",
    title_zh: "TapeOutScan 公开数据浏览器",
    summary_en: "Community block explorer for TapeOut on BNB Smart Chain, still self-declared as in beta. Its public front page runs a live protocol event feed \u2014 commits, tape-outs, leaderboard entries and exits, fraud calls, trades and transfers \u2014 each row carrying a block number, a timestamp and a BscScan transaction link, alongside a market strip quoting BNB, $BEM and NAND/LATCH for both TapeOut and Behemoth, and time-series charts for on-chain activity, network hashrate and mining addresses. The site publishes its own data policy, separating evidence read once at a circuit's first sighting from on-chain state it refreshes on a cycle and from market-derived figures it recomputes and never caches as evidence. Access boundary, checked directly: the front page is the public part. The per-circuit pages, the per-address pages and the leaderboard all redirect to a sign-in wall, and registration there takes a username and password plus an invite code to activate the account, so despite the site's name most of it is not open browsing today.",
    summary_zh: "面向 BNB Smart Chain 上 TapeOut 的社区区块浏览器，作者自述仍在内测。其公开首页是一条实时协议事件流——承诺、流片、上榜与掉榜、打假、买卖与转移——每条都带区块号、时间戳和 BscScan 交易链接，同时提供覆盖 TapeOut 与 Behemoth 的 BNB / $BEM / NAND / LATCH 行情条，以及链上活跃、全网算力与挖矿地址的时序图表。该站公开了自己的数据口径，把「电路首次发现时录入的固定证据」「按周期刷新的链上状态」与「每次重算、从不作为证据缓存的市场派生值」三者明确分开。实测确认的访问边界：公开的是首页。电路详情页、地址页与排行页都会跳转到登录墙，注册需要用户名密码，且新账户还需邀请码激活——所以尽管站名是「公开数据浏览器」，目前它的大部分内容并不能免登录浏览。",
    use_cases: ["analytics", "processor-observation", "community-research"],
    safety_en: "Third-party and non-profit by its own statement, unaffiliated with the protocol team, and labelled beta with a standing warning that its data and pages may be unstable \u2014 treat every figure as provisional and confirm anything that matters against the chain or the official site. Its market-derived values, including estimated daily output, manufacturing cost and static payback period, are that tool's own model output, not protocol guarantees, and are not adopted here. Creating an account to reach the gated pages means handing a community beta a username and password and obtaining an invite code; this review covers only what is readable without an account, and nothing behind that wall has been checked.",
    safety_zh: "作者自述为第三方公益项目、与项目方无关，且站点标注内测并长期挂着「数据与网页可能不稳定」的提示——所有数字都应视为暂定值，重要结论请回到链上或官网核对。它的市场派生值（预计日产、制造价、静态回本等）是该工具自己的模型结果，不是协议保证，本站不予采纳。要访问被拦住的页面就得给一个社区内测项目注册用户名和密码、并取得邀请码；本次审核只覆盖了无需登录即可阅读的部分，登录墙之后的内容我们一概没有核验。"
  },
  {
