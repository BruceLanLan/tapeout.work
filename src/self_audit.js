...
export async function ensureSelfAuditSchema(env) {
  if (!selfAuditSchemaReady) {
    selfAuditSchemaReady = env.DB.batch([
...
export async function syncContentDrift(env) {
  ...
  try {
    const existingRows = await env.DB.prepare("SELECT tool_id, asset_fingerprint, surface_fingerprint, change_count FROM tool_content_fingerprints").all();
    ...
    for (const [index, result] of results.entries()) {
      ...
      statements.push(env.DB.prepare(`INSERT INTO tool_content_fingerprints
          (tool_id, url, asset_fingerprint, surface_fingerprint, first_seen_at, last_checked_at, last_changed_at, change_count, last_status, last_error)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'ok', NULL)
...
export async function selfAuditOverview(env, localeCoverage = null) {
  ...
  const [fingerprintRows, latestRun] = await Promise.all([
    env.DB.prepare("SELECT tool_id, url, last_checked_at, last_changed_at, change_count, last_status, last_error FROM tool_content_fingerprints").all(),
  ...
  return {
    ...
    boundary: "This detects that a page changed, never what changed or whether our description is now wrong — a human decides that. A tool absent from the queue is not certified current: a server-rendered edit that ships no new build leaves both fingerprints untouched, and non-HTML destinations cannot be fingerprinted at all. Nothing here is automatically written into the catalogue.",
  };
}
