...
const [page, app, registry, assets] = await Promise.all([
  ...
  fetch(`${base}/app.js?v=2026-08-31-features-r31&registry-governance-qa=${Date.now()}`).then(response => response.text()),
  getJson('/api/v1/processors?q=BLONSKR&page=1&page_size=10'),
  getJson('/api/v1/official-assets/overview'),
]);
assert(page.includes('app.js?v=2026-08-31-features-r31'), 'r12 app asset is not referenced by production HTML');
