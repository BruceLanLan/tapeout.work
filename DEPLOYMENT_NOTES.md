...
## 2026-09-06 归档 RPC 对 Workers 出口彻底停摆，持币普查冻结 7.5 小时

发现过程值得记：本来是在给 firsto 条目写「它的持币数和我们的对不对得上」，去核这句话，才发现**我们自己的普查已经停了**——`census_status: error`，检查点 450 分钟没动。

事实：`bem_holder_sync_runs` 里 2026-09-05 09:15Z 之前全部 ok（40 次），09:20Z 之后**连续 90 次全部 error**，无一成功，错误都是 `RPC -32002: request timed out`。同一时刻从本机 curl bloXroute（`https://bsc.rpc.blxrbdn.com`）三次全部毫秒级返回。所以不是提供方挂了，是**它不再应答 Cloudflare 的共享出口**——和 GeckoTerminal、DexScreener 是同一类问题，只是这次是 100% 而不是限流。

另一个关键细节：`from_block` 是 `null`，说明失败发生在循环之前的 `eth_blockNumber` 上。之前加的「超时就把窗口减半重试」只包了 `eth_getLogs`，**第一个 RPC 调用没有任何保护**，一失败整轮就死。

处置分两半：

1. **代码**：`bem_holders.js` 新增 `holdersRpc()`，按 [归档, 市场] 顺序试端点。理由是**归档节点只有回溯历史时才需要**——创世回填 9/3 就做完了，此后普查只扫链头附近，而链头附近的 `eth_getLogs` 正是市场扫描一直在用 PublicNode 做的事。实测确认：PublicNode 能正常返回落后链头 2,100 块的窗口（911 条日志），但拒绝落后 8.5 小时的窗口（`Archive requests require a personal token`）。所以回退能救「跟得上时」的情况，救不了「已经落后很远」的情况——后者仍然失败，这是诚实的结果，那个缺口确实需要归档节点。
2. **数据**：缺口 60,573 块由本机 `scripts/backfill_bem_holders.mjs` 补回（按文档先撤密钥暂停 Worker 普查，回填完再放回）。结果：折算 26,382 笔 Transfer、383 个地址、**0 个负余额**，检查点推到 120,145,987，持币数 3,365 不变。

**留给未来的判断**：普查每轮扫 6,000 块、链每 5 分钟只涨约 400 块，所以只要多数轮次成功就能一直贴着链头，回退路径够用。但如果 bloXroute 长期不恢复、而普查又因为别的原因掉队超过 PublicNode 的非归档窗口，就只能再跑一次本机回填。真要彻底摆脱，得找第三家归档提供方或自建。
